  ...
  // --------------------------------------------------------------
    ...
    public function listar_consultorios()
    {
        ...
        if(  $this->auth_role == 'admin' )
        {
            ...
            $this->load->view('gestores/consultorios/vista_listar' , $data);
 ...
 */     
    ...
    public function listar_consultorios_ajax()
    {
        ...
        {       
			...
			//echo "search cole: " . $search_tipo_cole;exit;
		 	...
		 	$this->load->model('gestores/consultorios/Model_listar') ;
			
			//total de registros
			$totalData = $this->Model_listar->allconsultorios_count();
	        ...
	        {            
	            $posts = $this->Model_listar->allconsultorios($limit,$start,$order,$dir);
	        }
	        else {
	            $search = $this->input->post('search')['value']; 
	
	            $posts =  $this->Model_listar->consultorios_search($search,    $limit,$start,$order,$dir );
	
	            $totalFiltered = $this->Model_listar->consultorios_search_count($search);
	        }
	        ...
	        if(!empty($posts))
	        {
	        	...
	        	foreach ($posts as $post)
	            {
					...
					$nestedData['id_registro'] = $post->id_consultorio; //para hacer la eliminacion
					
					$nestedData['nombre_consultorio'] = $post->nombre_consul;
					...
					$nestedData['fecha_registro'] =  array(
														'mostrar_fecha' => fecha_transformar_fecha_sin_hora($post->fecha_registro_consul),
														'ordenar_fecha' => $post->fecha_registro_consul
													  );
