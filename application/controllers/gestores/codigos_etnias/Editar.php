...
class Editar extends MY_Controller {
 ...
 // --------------------------------------------------------------
     ...
     */   	 
	  ...
	  public function form_editar($id_registro = null){
        ...
        if(  $this->auth_role == 'admin' )
        {
        	 ...
        	 if($id_registro == null )
			  redirect('gestores/codigos_etnias/listar/listar_codigos');
         
         	 //cargar librería de formulario
         	 $this->load->helper('form');
			
         	 //obtener los datos del registro
         	 $this->load->model('gestores/codigos_etnias/Model_editar');
             ...
             $this->load->view('gestores/codigos_etnias/vista_editar', $data)  ;
	...
	private function mi_validacion_editar ($post) 
	{
		...
		//Para el codigo
		if($codigo->codigo_codet != $post['text_codigo'])
		{
		  //para el código
		...
		{
			$this->form_validation->set_rules('text_codigo', 'Código Etnia', 'required|trim|min_length[1]|max_length[20]' );
		}

        //Para la descripcion
        if($codigo->descripcion_codet != $post['text_descripcion'])
        {
