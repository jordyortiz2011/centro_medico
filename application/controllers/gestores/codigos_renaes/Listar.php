  ...
  // --------------------------------------------------------------
    ...
    public function listar_codigos()
    {
        ...
        if(  $this->auth_role == 'admin' )
        {
            ...
            $this->load->view('gestores/codigos_renaes/vista_listar' , $data);
    ...
    public function listar_codigos_ajax()
    {
        ...
        {       
			...
			//echo "search cole: " . $search_tipo_cole;exit;
		 	...
		 	$this->load->model('gestores/codigos_renaes/Model_listar') ;
	        ...
	        if(!empty($posts))
	        {
	        	...
	        	foreach ($posts as $post)
	            {
					...
					$nestedData['id_registro'] = $post->id_codigo_renaes; //para hacer la eliminacion
					
					$nestedData['cod_renaes_codren'] = $post->cod_renaes_codren;

                    $nestedData['micro_red_codren'] = $post->micro_red_codren;

				
	               
					$nestedData['fecha_registro'] =  array(
														'mostrar_fecha' => fecha_transformar_fecha_sin_hora($post->fecha_registro_codren),
														'ordenar_fecha' => $post->fecha_registro_codren
													  );
