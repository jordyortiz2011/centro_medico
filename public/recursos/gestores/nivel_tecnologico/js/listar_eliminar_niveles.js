       ...
       $('#listado_niveles').DataTable({
               ...
               // processing: true, 
                serverSide: true,                              
        	     ...
        	     ajax:{
        		            url : BASE_URL + "gestores/nivel_tecnologico/listar/listar_niveles_tecnologicos_ajax", // json datasource
        		            type: "post",  // type of method  , by default would be get
        		            error: function(xhr, ajaxOptions, thrownError){  // error handling code
        		             		 $("#listado_niveles").css("display","none");
        	                    ...
        	                    { data: null, orderable: false ,render: function ( data, type, row ) {
        				                 ...
        				                 // Opciones de la tabla                                                
                                            return "<div class='action-buttons' id='" + data.id_registro  + "' + > "+                                                    
                                                     "<a class='btn btn-outline-info' href='"+  BASE_URL+ "gestores/nivel_tecnologico/editar/form_editar/" + data.id_registro +"'"+
...
// ================== Filtrado ==================================    
    ...
    /***********--DELETE--***************/
    $("#listado_niveles").on('click', 'a.delete',function(e)
    {
      e.preventDefault();
            ...
            function(isConfirm){
              if (isConfirm) {
                //proceder a eliminar:
                 $.ajax({
                    type: "POST",
                    url: BASE_URL + "gestores/nivel_tecnologico/listar/eliminar_nivel_tecnologico_fisico/"+id ,
                    data: dataString,
                    cache: false,
                    success: function(e)
                    {
                        console.log(e);
                         ...
                         switch (String(e) ) {  
                             ...
                             case 'registro_usado' :
                                    swal("¡No se puede eliminar!", "El nivel tecnológico está siendo usado.", "error");break;
                             case 'eliminar_error' : 
