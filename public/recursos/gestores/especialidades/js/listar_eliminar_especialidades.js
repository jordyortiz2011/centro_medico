...
$(document).ready(function () {
  ...
  //$('#filtrado_tipo_cole').parent().find('.select2-container').css('width', ''); 
   ...
   function listar_tabla () { 
       $('#listado_especialidades').DataTable({
               ...
               // processing: true, 
                serverSide: true,                              
        	     ...
        	     ajax:{
        		            url : BASE_URL + "gestores/especialidades/listar/listar_especialidades_ajax", // json datasource
        		            type: "post",  // type of method  , by default would be get
        		            error: function(xhr, ajaxOptions, thrownError){  // error handling code
        		             		 $("#listado_especialidades").css("display","none");
        	                    ...
        	                    { data: null, orderable: false ,render: function ( data, type, row ) {
        				                 ...
        				                 // Opciones de la tabla                                                
                                            return "<div class='action-buttons' id='" + data.id_registro  + "' + > "+                                                    
                                                     "<a class='btn btn-outline-info' href='"+  BASE_URL+ "gestores/especialidades/editar/form_editar/" + data.id_registro +"'"+
...
// ================== Filtrado ==================================    
    ...
    /***********--DELETE--***************/
    $("#listado_especialidades").on('click', 'a.delete',function(e)
    {
      e.preventDefault();
            ...
            function(isConfirm){
              if (isConfirm) {
                //proceder a eliminar:
                 $.ajax({
                    type: "POST",
                    url: BASE_URL + "gestores/especialidades/listar/eliminar_especialidad_fisico/"+id ,
                    data: dataString,
                    cache: false,
                    success: function(e)
                    {
                        console.log(e);
                         ...
                         switch (String(e) ) {  
                             ...
                             case 'registro_usado' :
                                    swal("¡No se puede eliminar!", "La unidad está siendo usado.", "error");break;
                             case 'registro_usado_profesional' :
                                 swal("¡No se puede eliminar!", "La especialidad está asignada a un profesional.", "error");break;
                             case 'eliminar_error' : 
