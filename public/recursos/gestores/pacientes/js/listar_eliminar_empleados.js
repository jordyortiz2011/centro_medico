       ...
       $('#listado_empleados').DataTable({
               destroy : true,   //destruir la tabla y volver a crearla al hacer cambios
                ...
                serverSide: true,                              
        	     ...
        	     ajax:{
        		            url : BASE_URL + "gestores/empleados/listar/listar_empleados_ajax", // json datasource
        	                    ...
        	                    { data: null, orderable: false ,render: function ( data, type, row ) {
        				                 ...
        				                 // Opciones de la tabla                                                
                                            return "<div class='action-buttons' id='" + data.id_registro  + "' + > "+                                                    
                                                     "<a class='btn btn-outline-info' href='"+  BASE_URL+ "gestores/empleados/editar/form_editar/" + data.id_registro +"'"+ 
...
// ================== Filtrado ==================================    
     ...
     //configuraciones para el filtrado 
      ...
      //Columna no visible  
      $('#filtrado_tipo_emplea').on('change', function (evt) {
           console.log( this.value );    
           oTable = $('#listado_empleados').DataTable();            
    ...
    /***********--DELETE--***************/
    $("#listado_empleados").on('click', 'a.delete',function(e)
    {
      e.preventDefault();
            ...
            function(isConfirm){
              if (isConfirm) {
                //proceder a eliminar:
                 $.ajax({
                    type: "POST",
                    url: BASE_URL + "gestores/empleados/listar/eliminar_empleado_fisico/"+id ,
                    data: dataString,
                    cache: false,
                    success: function(e)
                    {
