...
$(document).ready(function () {
  ...
  //$('#filtrado_tipo_cole').parent().find('.select2-container').css('width', ''); 
   ...
   function listar_tabla () { 
       $('#listado_consultorios').DataTable({
               ...
               // processing: true, 
                serverSide: true,                              
        	     ...
        	     ajax:{
        		            url : BASE_URL + "gestores/consultorios/listar/listar_consultorios_ajax", // json datasource
        		            type: "post",  // type of method  , by default would be get
        		            error: function(xhr, ajaxOptions, thrownError){  // error handling code
        		             		 $("#listado_consultorios").css("display","none");
        	                    ...
        	                    { data: null, orderable: false ,render: function ( data, type, row ) {
        				                 ...
        				                 // Opciones de la tabla                                                
                                            return "<div class='action-buttons' id='" + data.id_registro  + "' + > "+                                                    
                                                     "<a class='btn btn-outline-info' href='"+  BASE_URL+ "gestores/consultorios/editar/form_editar/" + data.id_registro +"'"+
...
// ================== Filtrado ==================================    
    ...
    /***********--DELETE--***************/
    $("#listado_unidades").on('click', 'a.delete',function(e)
    {
      e.preventDefault();
            ...
            function(isConfirm){
              if (isConfirm) {
                //proceder a eliminar:
                 $.ajax({
                    type: "POST",
                    url: BASE_URL + "gestores/unidades/listar/eliminar_unidad_fisico/"+id ,
                    data: dataString,
                    cache: false,
                    success: function(e)
                    {
