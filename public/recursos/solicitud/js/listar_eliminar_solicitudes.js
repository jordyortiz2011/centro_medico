$(document).ready(function () {
  ...
  $('#filtrado_estado , .select2').select2({
       ...
       $('#listado_solicitudes').DataTable({
               destroy : true,   //destruir la tabla y volver a crearla al hacer cambios
                ...
                serverSide: true,                              
        	     ...
        	     ajax:{
        		            url : BASE_URL + "solicitud/listar/listar_solicitudes_ajax", // json datasource
                             data: {
                                 "id_estado"    : $('#filtrado_estado option:selected').val(), //para filtrado por defecto
        		 ...
        		 columns: [
...

                             {
                                 data: null, orderable: false, render: function (data, type, row) {
                                     ...
                                     options += "<a class='btn btn-outline-info' href='" + BASE_URL + "solicitud/editar/form_editar/" + data.id_registro + "'";
                                     options += " data-toggle='tooltip' title='Ver'>";
                                     options += "<i class=' fa fa-eye bigger-150'></i>";
                                     options += "</a> &nbsp;";
...
// ================== Filtrado ==================================    
     ...
     //configuraciones para el filtrado 
      ...
      //Columna no visible  
      $('#filtrado_estado').on('change', function (evt) {
    ...
    $("#listado_solicitudes").on('click', 'a.delete',function(e)
    {
      e.preventDefault();
       ...
       swal({
              ...
              text: "Se borrarán todos los datos asociados (Matriz, Secuencia, Estimación, Fotos, Post Desembolso)",
              type: "warning",
